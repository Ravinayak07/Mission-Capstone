# -*- coding: utf-8 -*-
"""Book_Model_Training_Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DAHJXDMWZ3noMPJfhzq8BKI9RHW0poYE
...
- The data utilized in this document was sourced from Kaggle.[https://www.kaggle.com/datasets/arashnic/book-recommendation-dataset]
- The Book-Crossing dataset consists of three files:
# Users
- This file includes user information. User IDs (User-ID) have been anonymized and are represented by integers. If available, demographic details (Location, Age) are provided; otherwise, these fields contain NULL values.
# Books
- Books are identified by their unique ISBNs. Invalid ISBNs have been excluded from the dataset. Additionally, some content-based information is available (Book-Title, Book-Author, Year-Of-Publication, Publisher), sourced from Amazon Web Services. Note that if there are multiple authors, only the first one is listed. URLs directing to cover images are also included, available in three sizes (Image-URL-S, Image-URL-M, Image-URL-L) - small, medium, and large. These URLs lead to the Amazon website.
# Ratings
- This file contains information about book ratings. Ratings (Book-Rating) are either explicit, expressed on a scale of 1-10 (with higher values indicating greater appreciation), or implicit, represented by 0.
...
plt.pie(values, labels=labels, autopct='%1.1f%%', startangle=140, colors=sns.color_palette('Set2'))
